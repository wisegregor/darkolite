...
# CONFIG
# ---------------------------------------------------
START_YEAR = 2025          # scrapes 2025-26
END_YEAR   = 2026          # stops before 2026-27
SAVE_LOCAL = True
OUTPUT_DIR = "./"          # same directory
...
def safe_fetch_leaguegamelog(season: str, season_type: str = "Regular Season") -> pd.DataFrame:
    """
    PATCH:
    - Try the requested season (e.g., "2025-26")
    - If NBA API returns empty, fall back to "Current"
    - Once NBA adds the season officially, the normal path works again
    """

    logging.info(f"Attempting to fetch game log for season '{season}'...")
    ...
    try:
        ...
        # Success case
        if not df.empty:
            logging.info(f"✓ Season '{season}' recognized by API — {len(df)} games found.")
            return df

        # Otherwise: fallback
        logging.warning(f"⚠️ Season '{season}' returned empty. Falling back to 'Current'.")
        df_current = leaguegamelog.LeagueGameLog(
            season="Current",
            season_type_all_star=season_type
        ).get_data_frames()[0]

        if df_current.empty:
            logging.error("❌ Even 'Current' returned empty — NBA API not returning data right now.")
        else:
            logging.info(f"✓ 'Current' returned {len(df_current)} games (using that instead).")

        return df_current

    except Exception as e:
        logging.error(f"❌ Failed to fetch logs for season '{season}': {e}")
        return pd.DataFrame()
...
def scrape_season(season: str):
    ...
    games = safe_fetch_leaguegamelog(season)

    if games.empty:
        logging.warning(f"⚠️ No games returned for season {season}, skipping...")
        return
